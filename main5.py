...
class VotingApp(QWidget):
    def __init__(self):
        ...
        self.admin_id = "59"  # Admin voter ID
        ...
        self.show_results_button.clicked.connect(self.check_admin_id)
    ...
    def vote(self, option):
        ...
        if not voter_id:
            QMessageBox.warning(self, "Error", "Please enter your voter ID!")
    ...
    def check_admin_id(self):
        voter_id = self.voter_id_input.text()
        if voter_id == self.admin_id:
            self.show_results()
        else:
            QMessageBox.warning(self, "Access Denied", "You are not authorized to view the results.")

    def show_results(self):
        final_results = f"Final Results:\nOption 1: {self.votes['Option 1']} votes\nOption 2: {self.votes['Option 2']} votes\nOption 3: {self.votes['Option 3']} votes"
        QMessageBox.information(self, "Voting Results", final_results)
